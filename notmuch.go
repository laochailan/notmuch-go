// Package notmuch provides Go bindings for the notmuch library.
package notmuch
...
func (self *Query) SearchThreads() *Threads {
	...
	status := C.notmuch_query_search_threads(self.query, &threads)
...
func (self *Query) SearchMessages() *Messages {
	...
	status := C.notmuch_query_search_messages(self.query, &msgs)
	if status != C.NOTMUCH_STATUS_SUCCESS {
		return nil
	}
...
func (self *Query) CountMessages() uint {
	var count C.uint
	status := C.notmuch_query_count_messages(self.query, &count)
	if status != C.NOTMUCH_STATUS_SUCCESS {
		return 0
	}
	return uint(count)
